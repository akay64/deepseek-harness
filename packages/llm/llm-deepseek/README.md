...
When [proactive compaction](../../compaction/compaction-basic/README.md#use-this-package) is enabled, `models[].contextWindow` (or `defaultContextWindow` when absent) determines its pressure trigger and retention budget without subtracting the request output cap or `headroomTokens`. A large output request can still exceed the provider's context window; lower `thresholdRatio` to compact earlier.
